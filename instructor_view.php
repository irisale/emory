...
<html>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') 
    {
        if (isset($_POST['course'])) { //user logging in
                $_SESSION["classID"] = $_POST['courseID'];
                header("location: instructor_class_stats.php");
        }
    }
    ?>
            ...
            <form id ="courseID" method="POST">
                <div class="form-group">
                    <?php
                        ...
                        $courseID_query="SELECT classID, sectionNumber, semester, year, courseID FROM class WHERE instructorID = '$instructorID' ORDER BY year ASC, semester ASC";
                        if($classresult = mysqli_query($conn, $courseID_query)){
                            if(mysqli_num_rows($classresult) > 0){ 
                                ...
                                while ($row = mysqli_fetch_array($classresult)) {
                                    ...
                                    echo '<option value="'.$classID.'"> '.$semester.''.$year.': '.$courseID.' - '.$sectionNumber.'</option>';
                                }
                                echo '</select>';
                            }
                            if(mysqli_num_rows($classresult) == 0){
